...
export function marquee(input: string, displayAmount = input.length, options: Options): SegmentName[][][] {
    const frames = [];
...
export function blink(input: string, counts = 10, options: Options): SegmentName[][][] {
    const frames = [];
...
export function typing(input: string, options: {convertToUpperCase: boolean, unknownChar: SegmentName[] | 'exception'}): SegmentName[][][] {
    const frames = [];
    const text = stringToDisplay(input, options);
